...
def _tolka_annonskort(kort) -> dict | None:
    ...
    try:
        ...
        print(f"[bytbil] kunde inte tolka annonskort: {e}")
        return None


def hamta_annonser() -> list[dict]:
    print("[bytbil] hämtar annonser...")
    try:
        resp = requests.get(SOK_URL, headers=HEADERS, timeout=15)
        resp.raise_for_status()
    except Exception as e:
        print(f"[bytbil] FEL vid hämtning: {e}. Verifiera riktig URL.")
        return []
    ...
    print(f"[bytbil] {len(bilar)} annonser matchade grundkraven")
    return bilar
